...
const AchievementProgress = ({ target, progress }) => {
  ...
  const [isEmpty, setIsEmpty] = useState({ bronze: true, silver: true, gold: true });
  ...
  useEffect(() => {
    ...
    const bronzeProgressPercent = bronzeProgress > 100 ? 100 : bronzeProgress;
    const silverProgressPercent = silverProgress > 100 ? 100 : silverProgress;
    ...
    if (progress > gold_count) {
      ...
      fillRef.current.style.background = 'linear-gradient(141.18deg, #de4b46, #cb2a24)';
      ...
      fillRef.current.style.background = 'linear-gradient(141.18deg, #de4b46, #cb2a24,  #cd672d, #a35224)';
      ...
      <div>
        ...
        <div className={emptyLine}>
          <div className={fill} ref={fillRef}>
            <div className={`${dot} ${first}`}>{/* <div className={redempty}></div>} */}</div>
            <div className={`${dot} ${bronze}`}>{isEmpty.bronze && <div className={bronzeempty}></div>}</div>
            <div className={`${dot} ${silver}`}>{isEmpty.silver && <div className={silverempty}></div>}</div>
            <div className={`${dot} ${gold}`}>{isEmpty.gold && <div className={goldempty}></div>}</div>
          </div>
        </div>
      </div>
    </div>
