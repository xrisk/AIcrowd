module SubmissionsHelper
  ...
  def show_submission_popup?(participant)
    participant.present? && rand(100) < 2 && participant.bio.blank? && participant.github.blank? && participant.twitter.blank? && participant.linkedin.blank? && participant.website.blank? && ((Time.now - (participant.submissions.order(:created_at)&.last&.created_at || Time.now - 26.hours)) <= 24.hours)
  end
end
