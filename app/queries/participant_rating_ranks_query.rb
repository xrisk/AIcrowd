class ParticipantRatingRanksQuery
  ...
  def call()
    @relation.where('fixed_rating is not null').map do |participant|
      user_final_rating = UserRating.where("participant_id=#{participant.id} and rating is not null").reorder('created_at desc').first
      ...
      UserRating.create(participant_id: participant.id, rating: updated_rating, variation: user_final_rating['variation'], challenge_round: user_final_rating['challenge_round'])
    end
