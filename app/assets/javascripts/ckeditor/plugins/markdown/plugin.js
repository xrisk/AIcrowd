        ...
        init: function(editor) {
            ...
            editor.addMode('markdown', function(callback) {
                ...
                if (typeof(TurndownService) == 'undefined') {
                    CKEDITOR.scriptLoader.load('//unpkg.com/turndown@6.0.0/dist/turndown.js', function() {
                      setupTurnDownMarkdown(editable, htmlData)
                    });
                } else {
                  setupTurnDownMarkdown(editable, htmlData)
                }
                ...
                if (typeof(showdown) == 'undefined') {
                    CKEDITOR.scriptLoader.load('//cdnjs.cloudflare.com/ajax/libs/showdown/1.9.1/showdown.min.js');
                }
    ...
    var sourceEditable = CKEDITOR.tools.createClass({
        ...
        proto: {
            ...
            detach: function() {
                ...
                editor.setData(converter.makeHtml(markdownSource, {langPrefix: 'language-'}));
        ...
        markdown: {
            ...
            exec: function(editor) {
              if (editor.mode === 'markdown'){
                ...
                let userSure = confirm('Switching to Markdown view will remove any custom HTML classes you may have added. Are you sure you want to continue?')
                if (userSure) {
...
function setupTurnDownMarkdown(editable, htmlData){
  let turndownService = new TurndownService({ headingStyle: 'atx' });
  turndownService = turndownService.keep(['span', 'figure']);
  turndownService = turndownService.addRule('heading', {
      filter: ['h2', 'h3', 'h4', 'h5', 'h6'],
      replacement: function (content, node) {
        let hLevel = Number(node.nodeName.charAt(1)) - 1;
  ...
  turndownService = turndownService.addRule('math', {
    filter: function (node, options) {
      return (
        node.nodeName === 'SPAN' &&
        node.getAttribute('class') === 'math-tex'
      )
    },
