...
RSpec.describe MarkdownService do
  describe 'simple markdown string' do
    ...
    it { expect(described_class.new(markdown: markdown).call).to eq(["<h3 id=\"some-markdown\">Some markdown</h3>\n", nil]) }
  end
  ...
  describe '#mentions_to_links' do
    ...
    it {
      expect(described_class.new(markdown: markdown).mentions_to_links(rendered_html: rendered_html, mentions_cache: mentions_cache))
  ...
  describe 'single mentions' do
    ...
    it {
      expect(described_class.new(markdown: markdown, mentions_cache: mentions_cache).call)
           .to eq(["<h3 id=\"some-markdownn-sean-some-text\">Some markdown\\n <a href='/participants/1' target='_blank'>@Sean</a> some text</h3>\n", [1]])
    }
  end
  ...
  describe 'markdown is nil' do
    ...
    it { expect(described_class.new(markdown: markdown).call).to eq([nil, nil]) }
  end
end
