...
Commontator.configure do |config|
  ...
  config.current_user_proc = ->(context) { context.current_participant }
  ...
  config.user_name_proc = ->(participant) {participant.name  }
  ...
  config.user_mentions_proc = ->(current_participant, thread, query) do
    current_participant.class.where('name LIKE ?', "#{query}%")
  end
