module SanitizationHelper
  ...
  def sanitize_html_with_attr(html_content, elements: [], attributes: {})
    Sanitize.fragment(html_content, Sanitize::Config.merge(Sanitize::Config::RELAXED, elements: Sanitize::Config::BASIC[:elements] + elements, attributes: attributes, :parser_options => {max_errors: -1, max_tree_depth: -1})).html_safe
  end
end
